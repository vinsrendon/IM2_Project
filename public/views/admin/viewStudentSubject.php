...
<body onload="getStudSub(<?php echo $_SESSION['stud_id_to_get_sub']?>,<?php echo $_SESSION['Role']?>),loadSubjects()">
    ...
    <div class="flex justify-around sm:px-3">    
        <div class="xl:w-4/5 w-full p-4 bg-white rounded-lg shadow-md">
            ...
            <input type="button" id="addSubBtn" value="ADD SUBJECT" onclick="addSubjectToUser(<?php echo $_SESSION['stud_id_to_get_sub']?>,<?php echo $_SESSION['Role']?>)" class="w-full bg-indigo-600 text-white py-2 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
        </div>
    </div>
